...
def fingerprint(channel_samples: List[int],
    ...
    channel_samples=np.float32(channel_samples)
    #print("***********************************")
    #print(channel_samples.shape)
    melspec_sum = librosa.feature.melspectrogram(y=channel_samples, 
                                       sr=44100, 
                                       n_fft=4096, hop_length=2048)
    #melspec_sum = np.sum(S, axis=0)
    #print("*****************melspec_sum********************")
    #print(melspec_sum)
    S_dB = librosa.power_to_db(melspec_sum, ref=np.max)

    # Apply log transform since specgram function returns linear array. 0s are excluded to avoid np warning.
    # arr2D = 10 * np.log10(arr2D, out=np.zeros_like(arr2D), where=(arr2D != 0))
    
    #S_dB = 10 * np.log10(S_dB, out=np.zeros_like(S_dB), where=(S_dB != 0))
    #print("*******S_dB*******")
    #print(S_dB.shape)
        ...
        for j in range(1, fan_value):
            if (i + j) < len(peaks):
                ...
                if MIN_HASH_TIME_DELTA <= t_delta <= MAX_HASH_TIME_DELTA:
                    ...
                    #here changed the calculation of hashing s
                    h = xxhash.xxh3_128(f"{str(freq1)}|{str(freq2)}|{str(t_delta)}".encode('utf-8'))
